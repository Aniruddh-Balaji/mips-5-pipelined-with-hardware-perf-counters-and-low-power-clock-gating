...
wire id_branch =!stall&&(id_opcode==6'b000100)&&(reg_rdata1==reg_rdata2);
wire [31:0] branch_target=IF_ID_pc+4+(id_imm_se<<2);
reg [31:0] ID_EX_pc;
...
wire stall;
wire load_use_stall;
assign load_use_stall=ID_EX_memread&&(ID_EX_rt != 0)&&((ID_EX_rt== id_rs)||(ID_EX_rt ==id_rt));
wire branch_stall;
assign branch_stall=(id_opcode==6'b000100)&&((ID_EX_regwrite&&(ID_EX_rd != 0)&&((ID_EX_rd==id_rs)||(ID_EX_rd==id_rt)))||(EX_MEM_regwrite&&(EX_MEM_rd!=0)&&((EX_MEM_rd== id_rs)||(EX_MEM_rd ==id_rt)))||(MEM_WB_regwrite&&(MEM_WB_rd!=0)&&((MEM_WB_rd==id_rs)||(MEM_WB_rd==id_rt))));
...
always @(posedge clk or posedge reset) begin
    if(reset) begin
        ...
        if(!stall&&!id_branch&&(IF_ID_instr!=32'h00000000))
...
always @(posedge regfile_gated_clk or posedge reset) begin
    if(reset) begin
        ...
        if(MEM_WB_regwrite&&(MEM_WB_rd!=0))
            regfile[MEM_WB_rd]<=(MEM_WB_memtoreg ?MEM_WB_memdata:MEM_WB_aluout);
    end
end
always @(*) begin
    forwardA=2'b00;
    ...
    if(EX_MEM_regwrite&&(EX_MEM_rd!=0)&&(EX_MEM_rd==ID_EX_rt))
        forwardB=2'b10;
    if(MEM_WB_regwrite&&(MEM_WB_rd!=0)&&(MEM_WB_rd==ID_EX_rs)&&!(EX_MEM_regwrite&&(EX_MEM_rd!=0)&&(EX_MEM_rd==ID_EX_rs)))
        forwardA=2'b01;
    if(MEM_WB_regwrite&&(MEM_WB_rd!=0)&&(MEM_WB_rd==ID_EX_rt) &&!(EX_MEM_regwrite&&(EX_MEM_rd!=0)&&(EX_MEM_rd==ID_EX_rt)))
        forwardB=2'b01;
end
always @(*) begin
    case(ID_EX_alu_ctrl)
